...
parser.add_argument('--maxCount', type=int, default=20)
parser.add_argument('--starter', type=int, default=1)
...
print "Start from {0}".format(datetime.now())

fout = open(args.file,"w");

for i in range(args.starter,args.maxCount+1):
    cmd=re.subn("\([0-9]+","("+str(i), sc)[0]
    cmd=cmd.replace("tltri_","tltri_"+str(i));
    fout.write(cmd+"\n");
...
print "End at {0}".format(datetime.now())
